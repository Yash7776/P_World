...
def add_new_item(request): 
    if request.method == "POST":
        ...
        if available == 'true':
            ...
            available = False
        StoreItem.objects.create(fk_vendor=vendor_obj ,fk_category = category_obj, item_name = title,item_price=charges,item_image = item_image,pet_type = pet_type,item_description = description, available_status = available ) 
...
def show_vendors_item(request):
    if request.session.get("user_type") == "Store" : 
        item_obj = StoreItem.objects.filter(fk_vendor__id = request.session.get("customer_id") ).order_by('-id') 
        context = { 
        'item_obj' : item_obj,
        }
...
def edit_store_item(request): 
    if request.method == "POST":
        ...
        if available == 'true':
            ...
            available = False 
        item_obj = StoreItem.objects.get(id=item_id)
