...
def order_history_api(request):
    try :
        ...
        for i in order_obj :  
            ...
            for j in item_obj :
                obj = StoreItem.objects.get(id = j['fk_item_id'])
...
def get_item_list_api(request):
    try :
        items = StoreItem.objects.filter(available_status = True).values().order_by('-id')   
        for i in items:
...
def place_order_api(request):
    try:
        ...
        with transaction.atomic():
            ...
            for i in items:
                item_obj = StoreItem.objects.select_related('fk_vendor').get(
                    id=i['item_id']
                )
...
def get_all_items_api(request):
    try:
        ...
        qs = StoreItem.objects.all().order_by('-id')
