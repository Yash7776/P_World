...
@permission_classes((AllowAny, ))
@csrf_exempt
def create_walker_account(request):
    try:
        data = request.data
        response = Response()
        ...
        return Response({'status':200 , "msg":'Account is Created Successfully', "walker_id":User_obj.id })
...
def login_walker_api(request):
    try:
        ...
        if User_Details.objects.filter(email = email, password = password).exists():
            ...
            return Response({'status':200, "msg":'Login Successfull', "walker_id":trainer_obj.id})
        else:
...
@permission_classes((AllowAny, ))
@csrf_exempt
def create_walker_service_api(request):
    try:
        data = request.data
        response = Response()
        walker_id = data['fk_doctor']
               
        if User_Details.objects.filter(id = walker_id).exists():   
        
            serializer = DoctorServiceSerializer(data = request.data)
            if serializer.is_valid():
                serializer.save()
              
                return Response({'status':200 , "msg":'walker Service is create', "walker_service_id": serializer.data['id']})
            else:
...
@permission_classes((AllowAny, ))
@csrf_exempt
def get_walker_service_api(request):
    try:
        data = request.data
        response = Response()
        walker_id = data['fk_doctor']
        ...
        if User_Details.objects.filter(id = walker_id).exists():
            ...
            trainer_obj = User_service.objects.filter(fk_doctor_id = walker_id)
            
            for i in trainer_obj:
                doc_dict['Walker_service_id'] = str(i.id) if i.id else ""
            ...
            return Response({'status':200 , "msg":'Walker Service List', "Service List" : doc_list})
        else:
...
@permission_classes((AllowAny, ))
@csrf_exempt
def delete_walker_service_api(request):
    try:
        data = request.data
        response = Response()    
        service_id = data['walker_service_id']
                
        if User_service.objects.filter(id = service_id).exists(): 
            
            update_doc_obj = User_service.objects.filter(id = service_id).delete()
            
            return Response({'status':200 , "msg":'Walker Service is deleted successfully'})           
        else:   
            return Response({'status':403 , "msg":'No Walker service found.'})
